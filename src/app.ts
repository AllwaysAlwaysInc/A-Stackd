...
export async function buildApp(options: BuildAppOptions = {}): Promise<FastifyInstance> {
  ...
  app.setErrorHandler((error: FastifyError, request, reply) => {
    ...
    // Simulated Sentry Error Monitoring Hook (P6 Backend Hardening)
    console.error("[SENTRY MONITOR] Captured Unhandled Exception:", {
  ...
  await app.register(ticketRoutes(store, config.blockedStates));
  await app.register(poolRoutes(store));
  await app.register(adminRoutes(store));
  await app.register(paymentsRoutes(store, config.nodeEnv === "development"));
  await app.register(addressRoutes(store, config.blockedStates));
